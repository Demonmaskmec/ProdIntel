...
from models import db, User, Procedure, Job
...
@auth_required
def procedure():
    return render_template('procedure.html', user=User.query.get(session['user_id']), procedures=Procedure.query.all())
...
def add_procedure_post():
    ...
    if user.is_prodmanager:
        procedure = Procedure(procedure_name=procedure_name, procedure_description=procedure_description, procedure_plantime=int(procedure_plantime), procedure_planmanpower=int(procedure_planmanpower), procedure_is_prod=True, procedure_is_store=False)
    elif user.is_storemanager:
        procedure = Procedure(procedure_name=procedure_name, procedure_description=procedure_description, procedure_plantime=int(procedure_plantime), procedure_planmanpower=int(procedure_planmanpower), procedure_is_prod=False, procedure_is_store=True)
...
def edit_procedure(id):
    return render_template('procedure/edit.html', user=User.query.get(session['user_id']), procedure=Procedure.query.get(id))
...
def delete_procedure(id):
    return render_template('procedure/delete.html', user=User.query.get(session['user_id']), procedure=Procedure.query.get(id))
...
def add_job_post():
    ...
    job = Job(job_name=job_name, job_description=job_description, deadline_date=date.fromisoformat(deadline_date), deadline_time=deadline_time)
    db.session.add(job)
    db.session.commit()
...
@auth_required
def edit_job(id):
    return render_template('job/edit.html', user=User.query.get(session['user_id']), job=Job.query.get(id))
...
def edit_job_post(id):
    ...
    job.deadline_time = deadline_time
    db.session.commit()
...
@auth_required
def delete_job(id):
    return render_template('job/delete.html', user=User.query.get(session['user_id']), job=Job.query.get(id))
