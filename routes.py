...
from models import db, User, Procedure
...
@auth_required
def procedure():
    return render_template('procedure.html', user=User.query.get(session['user_id']))
...
def add_procedure():
    user = User.query.get(session['user_id'])
    if not user.is_admin or not user.is_prodmanager:
        flash('You are not authorized to access this page.')
        return redirect(url_for('index'))
...
def add_procedure_post():
    ...
    procedure = Procedure(procedure_name=procedure_name, procedure_description=procedure_description, procedure_plantime=int(procedure_plantime), procedure_planmanpower=int(procedure_planmanpower))
